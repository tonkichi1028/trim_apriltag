...
class tracking_apriltag2(object):
	def __init__(self):
		# ROS
		rospy.init_node("tracking_apriltag2")
		rospy.on_shutdown(self.cleanup)

		self.bridge = CvBridge()
		self.image_pub = rospy.Publisher("/masking_image2", Image, queue_size=1)
		self.info_pub = rospy.Publisher("/masking_info2", CameraInfo, queue_size=1)
		self.tag_det = rospy.Subscriber('/tag_detections2',AprilTagDetectionArray,self.tag_camera_callback)
		self.tag_pos = rospy.Subscriber('/tag_position2',AprilTagDetectionPositionArray,self.tag_image_callback)
		sub1 = message_filters.Subscriber('/usb_cam2/image_raw', Image)
		sub2 = message_filters.Subscriber('/usb_cam2/camera_info', CameraInfo)
