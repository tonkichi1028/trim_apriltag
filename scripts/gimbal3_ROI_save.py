...
class tracking_apriltag(object):
	def __init__(self):
		...
		# Add the variable for data to save
		self.save_data_positions = []
	...
	def tag_image_callback(self, data_image):
		if len(data_image.detect_positions) >= 1:
			...
			if self.flag_image == 0:
				...
				self.save_data_positions.append([self.Position_now_image.x, self.Position_now_image.y])
	...
	def save_data(self):
		...
		data_path = os.path.join(experiment_folder, 'data.csv')
		with open(data_path, 'w', newline='') as f:
			writer = csv.writer(f)
			writer.writerows(self.save_data_positions)
